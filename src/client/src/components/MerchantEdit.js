...
//Test data.
import { locations, Messages, merchantTypes } from '../../test/DataSource.js';

//Actions
import { addMerchant, getUser, getMerchantValues, displayLoading, hideLoading, getMerchantByIdAndLevel } from '../actions.js';
        ...
        getMerchantByIdAndLevel(1)
            .then((res) => {
                ...
                if (res.statusCode == 200) {
                    ...
                    hashHistory("/dashboard/merchant/list")
                }
            })
            .fail((err) => {
                console.log(err);
            ...
            submitHandler: function (form) {
                ...
                addMerchant(data, user.token.token).then((res) => {
                    if (res.statusCode) {
                        ...
                        $("#output").empty().html(`<label>${res.messages}</label>`);
    ...
    render() {
        ...
        return (
            <div id="merchant-registration">
                <div className="card">
                    <h3 className="card-header">Cập Nhật Thông Tin Merchant</h3>
